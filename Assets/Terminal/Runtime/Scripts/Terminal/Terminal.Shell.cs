...
namespace Saro.Terminal
{
    ...
    public class Shell
    {
        ...
        // 命令历史记录
        private LitRingBuffer<string> m_CommandHistory;
        ...
        public Shell()
        {
            ...
            m_CommandHistory = new LitRingBuffer<string>(32);
            ...
            // 注册内置命令
            AddCommand(typeof(BuildInCommands));
            ...
            {
                Terminal.LogError("[Shell] " + e.Message);
            }
        ...
        public void RegisterType(Type type, TypeParser fn)
        {
            ...
            {
                Terminal.LogWarning("[Shell] Already contains this type: " + type);
                return;
            }
        ...
        public void ExecuteCommand(string commandLine)
        {
            ...
            if (args.Count <= 0)
            {
                Terminal.LogError("[Shell] Command shouln't be null");
                return;
            }
            ...
            {
                Terminal.LogError("[Shell] Can't find this command : " + commandStr);
            }
            else if (!command.IsValid())
            {
                Terminal.LogError("[Shell] This command is not valid : " + commandStr);
            }
            else
            {
                if (!command.IsParamsCountMatch(args.Count))
                {
                    Terminal.LogError($"[Shell] {commandStr} : Parameters count mismatch, expected count : {command.ParamsTypes.Length}. Type : {string.Join<object>(",", command.ParamsTypes)}");
                    return;
                }
                ...
                for (int i = 0; i < command.ParamsTypes.Length; i++)
                {
                    ...
                    {
                        Terminal.LogError("[Shell] This paramter type is unsupported : " + command.ParamsTypes[i].Name);
                        return;
                    }

                    if (typeParse?.Invoke(args.Peek(), out paramters[i]) == false)
                    {
                        Terminal.LogError($"[Shell] Can't parse {args.Peek()} to type {command.ParamsTypes[i].Name}");
                    }
        ...
        private void InternalAddCommand(string command, string description, MethodInfo methodInfo, object instance)
        {
            ...
            foreach (char chr in command)
            {
                ...
                {
                    Terminal.LogError("[Shell] invalid characters : " + string.Join(",", m_InvalidChrsForCommandName));
                    return;
                }
            }
            ...
            for (int i = 0; i < parameters.Length; i++)
            {
                ...
                else
                {
                    ...
                    Terminal.LogError("[Shell] Unsupported type : " + type);
                    return;
                }
            }
